...
async function detailedExample() {
  try {
    ...
    const topTurnover = await nse.getTopTurnoverByValue();
    ...
    const tcsDetails = await nse.getEquityDetails('TCS');
    ...
    const startDate1Week = new Date();
    startDate1Week.setDate(startDate1Week.getDate() - 7);

    const startDate1Month = new Date();
    startDate1Month.setMonth(startDate1Month.getMonth() - 1);

    const data1Week = await nse.getEquityHistoricalData('TCS', {
      start: startDate1Week,
      end: endDate
    });

    const data1Month = await nse.getEquityHistoricalData('TCS', {
      start: startDate1Month,
      end: endDate
    });
    ...
    const infyCorporate = await nse.getEquityCorporateInfo('INFY');
    ...
    // 7. Informasi Index (semua index)
    console.log('7. Mendapatkan semua index...');
    const indices = await nse.getEquityStockIndices();
    console.log(`Total index tersedia: ${indices.data?.length || 0}`);
    if (indices.data && indices.data.length > 0) {
      console.log('Contoh 5 index pertama:', indices.data.slice(0, 5).map(idx => idx.index));
    }
    console.log('\n---\n');
